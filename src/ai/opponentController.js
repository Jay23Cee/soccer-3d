...
export function updateOpponentController({
  ...
  const targetGoal = [0, 0, -FIELD_CONFIG.LENGTH / 2 + 2];
  ...
  const inShootRange = Math.abs(currentPosition[2] - targetGoal[2]) <= AI_CONFIG.SHOOT_DISTANCE;
  ...
  switch (mode) {
    case OPPONENT_STATES.SHOOT: {
      const toGoal = normalize2D([targetGoal[0] - currentPosition[0], targetGoal[2] - currentPosition[2]]);
